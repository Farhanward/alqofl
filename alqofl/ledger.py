...
DEFAULT_LEDGER = Path("ledger") / "alqofl-ledger.jsonl"
DEFAULT_SECRET = Path("ledger") / ".alqofl-secret"
...
def append(payload: dict[str, Any], ledger_path: str | Path = DEFAULT_LEDGER) -> dict[str, Any]:
    ...
    record_hash = sha256(canonical(base))
...
def verify_ledger(ledger_path: str | Path = DEFAULT_LEDGER) -> dict[str, Any]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if record.get("payload_hash") != sha256(canonical(record.get("payload"))):
            return {"ok": False, "line": line_number, "message": "payload hash mismatch"}
        if record.get("record_hash") != sha256(canonical(base)):
